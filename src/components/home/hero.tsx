...
export default function Hero() {
  return (
    <section className='container relative flex h-full flex-col items-center justify-around gap-1 p-2 sm:gap-28'>
      <div className='flex flex-col items-center gap-2 pb-6 sm:flex-row sm:items-start sm:justify-around xl:justify-center'>
        <div className='flex flex-col mobilemd:gap-2 tablet:gap-3 sm:w-1/2 sm:self-center'>
          ...
          <motion.div whileHover={{ scale: 1.1, transition: { duration: 1 }}} whileTap={{ scale: 0.8, transition: { duration: 1 } }} >
            <Link href={siteConfig.mainNav[2].href} className={`hidden sm:flex ${buttonVariants({variant: 'secondary'})}`}>
              Book a Free Consultation →
            </Link>
          </motion.div>
        </div>
